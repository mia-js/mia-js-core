...
module.exports = function () {
    ...
    var _connect = function (dbName, url, options) {
        ...
        MongoClient.connect(url, options, function (err, db) {
            if (err) {
                Logger('err', "Could not connect to database '" + dbName + "'. Extended error information: " + err);
                deferred.reject(err);
            }
            else {
    ...
    var _close = function (dbName, db) {
        ...
        db.close(function (err, done) {
            if (err) {
                Logger('err', "Could not close connect to database '" + dbName + "'. Extended error information: " + err);
                deferred.reject(err);
            }
            else {
