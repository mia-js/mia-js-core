var Trycatch = require('trycatch')
    ...
    , ErrorHandler = require("mia-js-core/node_modules/errorHandler")
    , Logger = require('mia-js-core/node_modules/logger')
    , _ = require('lodash')
    , Shared = require('mia-js-core/node_modules/shared')
    , Translator = require('mia-js-core/node_modules/translator')
        ...
        _.forEach(methods, function (methodValue, method) {
            ...
            _.forEach(prefix, function (thisPrefix) {
                ...
                if (_isDuplicateIdentity(hostId, prefix, methodValue.identity)) {
                    //Logger('err', 'ERROR ' + methodValue.identity + '-' + method + ' for '+prefix+' ->  already registerd');
                    ...
                    if (!_isDuplicateRoute(thisPath, requestMethod)) {
                        ...
                        _.forEach(methodValue.controller, function (controller) {
                            ...
                            if (methodFunction.function) {
                                ...
                                Logger('err', '-> ' + controller.name + ' ' + controller.version + '::' + methodFunctionName + '<-- NOT FOUND');
    ...
    var _parseRoutes = function (routeConfig) {
        ...
        for (var i in hostIds) {
            ...
            if (routes) {
                ...
                Logger('err', 'ERROR No routes found');
            }
        }
    };
    ...
    var _parseRouteConfig = function () {
        _.forEach(Shared.routesConfig(), function (routeConfig, key) {
                if (!routeConfig.group) {
                    Logger('err', 'Missing group field for routes file');
                }

                if (!routeConfig.version) {
                    Logger('err', 'Missing version field for routes file');
                }
    ...
    self.initializeRoutes = function (vhosts) {
        ...
        for (var thisVhost in _vhosts) {
            ...
            _vhosts[thisVhost]["express"].use(function (err, req, res, next) {
                if (err && err.name == "ApplicationException") {
                    Logger('err', String.objToString(err));
                }
