    ...
    var BaseCronJob = BaseClass.extend({
        ...
        },
        {
            ...
            staticConstructor: function () {
                ...
                var _presetConfigToDbConfig = function () {
                    return {
                ...
                self.initializeJob = function (serverUniqueId) {
                    if (!_isDbInitialized) {
                        ...
                        return CronJobExecutionModel.validate(_parsePresetConfig()).fail(function (err) {
                            Logger('err', "Cron: '" + self.identity + "' has erroneous preset config");
                            return Q.reject(err);
                        }).then(function () {
                            return JobManagementDbConnector.getOrCreateDbJobEntry(self.identity, _presetConfigToDbConfig());
                        }).then(function (jobInfo) {
                            var dbConfig = _parseDbConfig(jobInfo);
                            return CronJobExecutionModel.validate(dbConfig).then(function (dbConfig) {
                                return Q(dbConfig);
                            }, function (err) {
                                Logger('err', "Cron: '" + self.identity + "' has erroneous database config. Ignoring db config and using preset config instead.");
                ...
                var _startOrRestartTimer = function () {
                    if (!_actualJobConfig) {
                        ...
                        if (!_canStartOnThisServer()) {
                            Logger('info', "Cron: '" + self.identity + "' is not allowed to run on this host");
                            return Q();
                        }
                        else {
                            if (!_cronJobTrigger) {
                                return Q().then(function () {
                                    ...
                                    Logger('info', "Cron: '" + self.identity + "' is started with " + self.getConfigOutput());
                                    return Q();
                                });
                            }
                            else {
                                ...
                                Logger('info', "Cron: '" + self.identity + "' detected config update with " + self.getConfigOutput());
                                return Q();
                            }
                        }
                    }
                ...
                self.onServerDeletedHandler = function(err) {
                    if (_actualJobConfig.debugOutput === true && _informAboutMissingServerOnce && !_hasInformedAboutMissingServer) {
                        Logger('info', "Cron: '" + self.identity + "' won't start, since the server is either paused or the server entry was removed from DB.");
                                    ...
                                    return JobManagementDbConnector.stopJob(jobId, timeLapsed).then(function (result) {
                                        if (_actualJobConfig.debugOutput === true) {
                                            if (result > 0) {
                                                ...
                                                Logger('err', "Cron: '" + self.identity + "' could not find/remove entry from db after finishing job instance.");
                                            }
                                        }
                                    });
                                });
                ...
                var _outputJobStats = function (jobDoc, serverUniqueId, started, timeLapsed) {
                    ...
                    Logger('info', "Cron: '" + self.identity + (started ? "' +1" : "' -1") + " active instances on this server: " + stats.runningThisServerCount + '/' + _actualJobConfig.maxInstanceNumberPerServer
