...
function thisModule() {
    ...
    self.initializeModels = function () {
        ...
        _addCoreModels([
            '../../../lib/cronJobs/lib/cronJobExecutionModel',
            '../../../lib/cronJobs/lib/cronServerHeartbeatModel'
        ]);
    };

    /**
     * Adds specified core models to shared lib
     * @param {Array} coreModels
     * @private
     */
    var _addCoreModels = function (coreModels) {
        coreModels.forEach(function (modelPath) {
            var model = require(modelPath);
            _shared.models[model.identity] = {};
            _shared.models[model.identity][model.version] = model;
        });
    }
    ...
    self.memcached = function (flush) {
        if (!_memcached) {
            ...
            if (env.defaultCache && env.defaultCache == "memcached" && !env.memcached) {
                throw new Error("Memcached config settings \'memcached\' missing in environment config file");
            }
    ...
    self.redis = function (flush) {
        if (!_redis) {
            ...
            if (env.defaultCache && env.defaultCache == "redis" && !env.redis) {
                throw new Error("Redis cache config settings \'redis\' missing in environment config file");
            }
            ...
            if (redisSettings) {
                ...
                if (redisSettings.clusterServers && _.isArray(redisSettings.clusterServers)) {
                    if (redisSettings.clusterServers.length == 0) {
                        throw new Error("Redis clusterServers is an empty array. Provide at least one server");
                    }
                ...
                if (flush == true && redisSettings.flushOnStart === true) {
                    _redis.flushdb(function (err, done) {
                        if (!err) {
