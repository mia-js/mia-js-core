var BodyParser = require('body-parser')
    ...
    , ParameterOverflow = require('./parameterOverflow')
...
function thisModule() {
    ...
    var _registeredServices = function (hostId, routeConfig, methodValue, method, path) {
        ...
        var methodParams = self.convertMethod(method)
            ...
            bodyParser: methodValue.bodyParser,
            url: path,
...
//        });
    ...
    var _parsePreConditionsParameters = function (preconditions) {
        ...
        var allowedAttributes = ["type", "desc", "subType", "required", "maxLength", "minLength", "match", "max", "min", "allow", "deny", "docs"];
                ...
                for (var name in parameters[section]) {
                        ...
                        for (var attr in addCondition) {
                            if (_.has(parametersList, [section, name, attr]) === false) {
                                ...
                                if (JSON.stringify(parametersList[section][name][attr]) != JSON.stringify(addCondition[attr])) {
                                    throw new MiaError("Precondition conflict: " + preconditions[index].name + ' -> ' + preconditions[index].version + ' -> ' + preconditions[index].method + ' -> ' + attr + ':' + addCondition[attr] + ' ->  already defined in previous controller and value is mismatching');
                                }
                                else {
                                    parametersList[section][name][attr] = addCondition[attr];
                                }
                    ...
                    if (_.has(parametersList, [section, name])) {
                        for (var attr in parametersList[section][name]) {
                            const validatedAttribute = _removeAttributes(parametersList[section][name][attr], attr);
                            if (!_.isUndefined(validatedAttribute)) {
                                validatedParameterAttributes[attr] = validatedAttribute
                            }
                        }
                    }
    ...
    var _isDuplicateRoute = function (path, requestMethod) {
        ...
        _.forEach(Shared.registeredServices(), function (regService) {
            if (regService.requestMethod) {
                if (regService.url == path && regService.requestMethod == requestMethod.toUpperCase()) {
                    set = true;
                }
            }
        });
    ...
    self.convertMethod = function (method) {
    ...
    var _getMethodFunction = function (controller, version, method) {
        ...
        if (currentController) {
            if (currentController[method] && _.isFunction(currentController[method])) {
                ...
                throw new MiaError(controller + ' ' + version + '->' + method + ' does not exists but used in routes configuration');
            }
    ...
    var _registerRoute = function (hostId, routeConfig, requestMethod, method, path, methodFunction, controller) {
        ...
        var innerRequestFunction = function (req, res, next) {
            ...
            // Add route information to req (only if not already there)
            // Don't do it every time because we might overwrite it from the outside (eg. forwarding)
            if (!req.miajs.route) {
                req.miajs.route = {
                    hostId: hostId,
                    group: (thisConfig.group),
                    ...
                    deprecated: (thisConfig.methodValue.deprecated || thisConfig.deprecated) === true,
                    method: method,
                    rateLimits: thisConfig.rateLimits || null,
                    parameterOverflow: thisConfig.methodValue.parameterOverflow || thisConfig.parameterOverflow || 'ignore'
                };
            }
            ...
            return methodFunction(req, res, next);
        };
        ...
        if (env.tryCatchForRouteFunctions) {
            requestFunction = async function (req, res, next) {
                try {
                    await innerRequestFunction(req, res, next)
                } catch (err) {
                    Logger.tag("UncaughtError").error(err);
                    const errorMsg = env.debug ? err : 'Something went wrong!'
                    next(new MiaError({'status': 500, name: 'ApplicationException', raw: errorMsg}), req, res);
                }
            }
        }
        else {
    ...
    var _validateRateLimits = function (routeConfig, methodValue) {
        ...
        if (globalRateLimit) {
            if (globalRateLimit.interval && _.isNumber(globalRateLimit.interval) && parseInt(globalRateLimit.interval) > 0 && globalRateLimit.maxRequests && _.isNumber(globalRateLimit.maxRequests) && parseInt(globalRateLimit.maxRequests) > 0) {
                ...
                throw new MiaError('Global rate limit config invalid' + ". Provide parameters 'interval' and 'maxRequests' with Int value");
            }
        }
        if (routeConfig && routeConfig.rateLimit) {
            if (routeConfig.rateLimit.interval && _.isNumber(routeConfig.rateLimit.interval) && parseInt(routeConfig.rateLimit.interval) > 0 && routeConfig.rateLimit.maxRequests && _.isNumber(routeConfig.rateLimit.maxRequests) && parseInt(routeConfig.rateLimit.maxRequests) > 0) {
                ...
                throw new MiaError('Rate limit config invalid for route file ' + routeConfig.prefix + ". Provide parameters 'interval' and 'maxRequests' with Int value");
            }
        }
        if (methodValue && methodValue.rateLimit) {
            if (methodValue.rateLimit.interval && _.isNumber(methodValue.rateLimit.interval) && parseInt(methodValue.rateLimit.interval) > 0 && methodValue.rateLimit.maxRequests && _.isNumber(methodValue.rateLimit.maxRequests) && parseInt(methodValue.rateLimit.maxRequests) > 0) {
                ...
                throw new MiaError('Rate limit config invalid for route ' + methodValue.identity + ' in route file ' + routeConfig.prefix + ". Provide parameters 'interval' and 'maxRequests' with Int value");
            }
        }

        if (!_.isEmpty(rateLimits) && !Shared.memcached()) {
            throw new MiaError("Rate limits set but memcached not configured. Provide settings for memcached in environment config file");
        }
    ...
    var _parseMethods = function (hostId, routeConfig, path, methods) {
        ...
        _.forEach(methods, function (methodValue, method) {
            methodParams = self.convertMethod(method);
            requestMethod = methodParams.requestMethod;
            ...
            thisRouteConfig.methodValue = methodValue;
            ...
            _.forEach(prefix, function (thisPrefix) {
                ...
                if (_isDuplicateIdentity(hostId, prefix, methodValue.identity)) {
                    throw new MiaError(methodValue.identity + ' for ' + prefix + ' ->  already registered');
                }
                else {
                    if (!_isDuplicateRoute(thisPath, requestMethod)) {
                        ...
                        _logInfo(requestMethod.toUpperCase() + ' ' + globalPath);
                        ...
                        _logInfo('-> PreconditionsCheck');

                        // Register parameter overflow controller
                        routeFunctions = routeFunctions.concat(_registerRoute(hostId, thisRouteConfig, requestMethod, method, thisPath, ParameterOverflow, {
                            name: 'checkParameterOverflow',
                            version: '1.0'
                        }));
                        _logInfo('-> ParameterOverflowCheck');
                        ...
                        _.forEach(methodValue.controller, function (controller) {
                            ...
                            if (_.isUndefined(methodFunction)) {
                                throw new MiaError('Controller ' + controller.name + ' ' + controller.version + '->' + methodFunctionName + ' does not exists');
                            }
                            ...
                            if (methodFunction.function) {
                                ...
                                if (controllerPreconditions && (controllerPreconditions[methodFunction.method])) {
                                    ...
                                    if (controllerPreconditions[methodFunction.method].parameters && !controllerPreconditions[methodFunction.method].responses) {
                                        throw new MiaError('Precondition parameters defined in ' + controller.name + ' ' + controller.version + '->' + methodFunction.method + ' but no responses defined');
                                    }
                                ...
                                //_logInfo(requestMethod.toUpperCase() + ' ' + thisPath + '-> ParameterPreloader');
                                ...
                                _logInfo('-> ' + controller.name + ' ' + controller.version + '::' + methodFunctionName);
                            }
                            else {
                                Logger.error('-> ' + controller.name + ' ' + controller.version + '::' + methodFunctionName + '<-- NOT FOUND');
                                throw new MiaError("Missing controller file!");
                            }
                        });
                        ...
                        var setCrossDomainHeaders = function (req, res, next) {
                            ...
                            if (req.method == 'OPTIONS') {
                                if (req.headers['access-control-request-method'] && ['GET', 'POST', 'PUT', 'DELETE'].indexOf((req.headers['access-control-request-method']).toUpperCase()) > -1) {
				                	res.setHeader('Access-Control-Allow-Methods', (req.headers['access-control-request-method']).toUpperCase() + ', OPTIONS')
                				}
                        ...
                        if (!_.isEmpty(corsHeaders)) {
                            _logInfo('Added CORS headers to route');
                            _vhosts[hostId]["router"]["options"].apply(_vhosts[hostId]["router"], [thisPath].concat(setCrossDomainHeaders));
                        ...
                        _vhosts[hostId]["router"][requestMethod].apply(_vhosts[hostId]["router"], routeArguments);
                        _logInfo('------------------------------------------------------------------------------------');
                    }
                    else {
                        throw new MiaError(requestMethod.toUpperCase() + ' ' + thisPath + '->' + methodValue.identity + ' already registered');
                    }
    ...
    var _parseRoutes = function (routeConfig) {
        ...
        if (!_.isArray(hostIds)) {
            throw new MiaError('Unknown definition type for host used in routes definiton. Use string or array');
        }
        ...
        for (var i in hostIds) {
            ...
            if (hostId != "*") {
                if (_.isEmpty(_vhosts[hostId])) {
                    throw new MiaError('Host ' + hostId + ' for route ' + routeConfig.group + ' not registered in mia.js global configuration.');
                }
            }
            ...
            if (routes) {
                ...
                _.forEach(routeConfig.prefix, function (thisPrefix) {
                    assetsDir = routeConfig.projectDir + path.sep + Shared.config("system.path.modules.public");
                    if (fs.existsSync(assetsDir)) {

                        if (thisPrefix == "/") {
                            _vhosts[hostId]["router"].use(Shared.express().static(assetsDir));
                        }
                        else {
                            _vhosts[hostId]["router"].use(thisPrefix + path.sep, Shared.express().static(assetsDir));
                        }
                        _logInfo('ASSETS ' + thisPrefix + '/->' + assetsDir);
                    }
                });
    ...
    var _parseRouteConfig = function () {
        _.forEach(Shared.routesConfig(), function (routeConfig, key) {
                ...
                if (routeConfig.routes) {
                    ...
                    // Check if route should be activated for current environment
                    if (routeConfig.environment) {
                        ...
                        if (_.indexOf(routeConfig.environment, Shared.config("environment.mode")) != -1) {
                            _logInfo('Creating routes for ' + routeConfig.group + ', Version: ' + routeConfig.version);
                            _parseRoutes(routeConfig);
                        }
                        else {
                            _logInfo('Ignore routes for ' + routeConfig.group + ', Version: ' + routeConfig.version + ' due to route is not allowed for current environment mode (' + Shared.config("environment.mode") + ')');
                        }
                    }
                    else {
                        _logInfo('Creating routes for ' + routeConfig.group + ', Version: ' + routeConfig.version);
    ...
    self.initializeRoutes = async function (vhosts, verbose = true) {
        const envConfig = Shared.config('environment');
        const swaggerMode = ['true', '1', 'y', 'yes', 'on', 'undefined'].includes(String(envConfig.swaggerMode).toLowerCase());
        _vhosts = vhosts;
        _verbose = verbose;
        //parse route config
        _parseRouteConfig();
        ...
        if (swaggerMode) {
            await Swagger.register(_vhosts, verbose);
        }
        ...
        return Promise.resolve()
            .then(() => {
                for (var thisVhost in _vhosts) {
                    ...
                    _vhosts[thisVhost]["router"].use(function (req, res, next) {
                        next(new MiaError({'status': 404}), req, res);
                    });

                    // Define global error handler
                    _vhosts[thisVhost]["router"].use(function (err, req, res, next) {
