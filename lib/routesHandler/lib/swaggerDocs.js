            ...
            for (var parameterName in obj) {
                ...
                if (_.isObject(parameterValue) && !_.isDate(parameterValue) && !_.isBoolean(parameterValue) && !_.isString(parameterValue) && !_.isNumber(parameterValue) && !_.isFunction(parameterValue) && !_.isRegExp(parameterValue) && !_.isArray(parameterValue)) {
                    ...
                    if (!_.isEmpty(subSchemaObject.attributes)) {
                        ...
                        _registeredSchemaDefs.push(definitionId);
            ...
            if (service.docs == true && service.hostId == hostId && service.url.match(/^\//)) {
                ...
                if (!_.isEmpty(responses)) {
                    ...
                    if (!_.isEmpty(parameters)) {
                        for (var sectionName in parameters) {
                            ...
                            if (sectionName != "body") {
                                ...
                                _registeredSchemaDefs.push(thisHash);
                    ...
                    for (var responseCode in responses) {
                        if (parseInt(responseCode) == 204) {
                            ...
                            if (parseInt(responseCode) < 300) {
                                if (successHash) {
                                    _registeredSchemaDefs.push(successHash);
                                    responseCodes[responseCode] = {
                                        "description": responses[responseCode].join(', '),
                                        "schema": {
                                ...
                                if (errorHash) {
                                    _registeredSchemaDefs.push(errorHash);
                                    responseCodes[responseCode] = {
                                        "description": responses[responseCode].join(', '),
                                        "schema": {
                    ...
                    for (var sIndex in bodySchemaModels) {
                        if (_registeredSchemaDefs.indexOf(bodySchemaModels[sIndex].name)  > -1) {
        ...
        return Promise.all(Object.keys(hosts).map((hostId) => {
          ...
          if (!_.isEmpty(swaggerDocResult)) {
            return SwaggerParser.validate(swaggerDocResult)
              .then((swaggerSpec) => {
                const swaggerUIIndex = fs.readFileSync(`${Path.dirname(require.resolve('swagger-ui'))}/index.html`, 'utf8');
                const swaggerUIIndexWithPath = swaggerUIIndex.replace(/url = "(.*)"/, `url = window.location.protocol + '//' + window.location.host + '/api-docs'`);

                hosts[hostId]["router"].get('/api-docs', (req, res) => res.json(swaggerSpec));
                hosts[hostId]["router"].get('/docs', (req, res) => res.send(swaggerUIIndexWithPath));
                hosts[hostId]["router"].use('/docs', Shared.express().static(Path.dirname(require.resolve('swagger-ui'))));
              })
              .catch((err) => {
                  console.error(err);
                  process.exit(1);
