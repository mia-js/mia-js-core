        ...
        staticConstructor:  function ()  {
            ...
            const _cleanServers =  () => {
                ...
                return JobManagementDbConnector.doHeartbeat(initialServerUniqueId).catch(err => {
                    ...
                    return CronJobExecutionModel.deleteMany({
                        typeName: {$nin: allCurrentCronsIdentities},
                        $or: [
                            // Didn't start at least once
                            {'stats.lastStarted.time': {$exists: false}},
                            // Last start is older than 1 hour
                            {'stats.lastStarted.time': {$gte: new Date(new Date().getTime() - 60 * 60 * 1000).toISOString()}},
                        ]
                    })
                });
            };
