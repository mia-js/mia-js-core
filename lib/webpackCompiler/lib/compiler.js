...
function Compiler(LoggerTagLess, Shared, config) {
    ...
    self.compileServer = () => {
        ...
        if (!fs.existsSync(configFilePath)) {
            return Q.reject('Webpack config file for development server not found - Skipping compilation!');
    ...
    const _compileProject = (projectConfig) => {
        ...
        if (Shared.runtimeArgs.hmr) {
            ...
            return _compileAndWatchBundle(clientWatchConfig, serverWatchConfig)
    ...
    const _purgePriorBundleFiles = (clientConfig, serverConfig) => {
        ...
        if (clientConfig) {
            files.push(path.dirname(path.join(clientConfig.output.path, clientConfig.output.filename)));
        }
        if (serverConfig) {
