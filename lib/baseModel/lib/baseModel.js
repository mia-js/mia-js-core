...
const _skipDatabaseIndexing = () => {
    const shouldSkip = process.env.skipDatabaseIndexing;
    const skipping = ['true', '1', 'y', 'yes'].includes(String(shouldSkip).toLowerCase())
    return skipping;
}
    ...
    var baseModel = BaseClass.extend({},
        ...
        //==================================
        {
            ...
            ensureIndexes: function (indexes, background = false, callback) {
                var self = this;
                if (_skipDatabaseIndexing()) { Logger.info(`skipping indexing for collection: ${self.identity}`); return Q(); }
