                ...
                Async.waterfall([
                    ...
                    function (db, next) {
                        ...
                        if (!db || _.isEmpty(db)) {
                            ...
                            Q(db.listCollections({name: self.collectionName}, {nameOnly: true}))
                                .then(cursor => cursor.toArray())
                                .then(async (collections) => {
                                    if (_.isEmpty(collections)) {
                                        const collectionOptions = self.collectionOptions || {}
                                        db.createCollection(self.collectionName, collectionOptions, (err, collection) => {
                                            if (!err) {
                                                next(null, collection)
                                            } else {
                                                db.collection(self.collectionName, (err, collection) => {
                                                    if (!err) {
                                        ...
                                        // collection exists already
                                        const collection = await db.collection(self.collectionName)
                                        self._collection = collection
                                        // do not apply indexes to exiting collection, since this operation is time-consuming
                                        next(breakExec, collection)
                                    }
