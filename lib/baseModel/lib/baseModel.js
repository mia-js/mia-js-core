...
const _isInStrictMode = () => {
    const strict = _.get(Shared.config('environment'), 'mongoStrictMode', true);
    return ['true', '1', 'y', 'yes', 'on'].includes(String(strict).toLowerCase());
}
                ...
                Async.waterfall([
                    ...
                    function (db, next) {
                        ...
                        if (!db || _.isEmpty(db)) {
                            ...
                            db.collection(self.collectionName, {strict: _isInStrictMode()}, function (err, collection) {
                                if (!err) {
