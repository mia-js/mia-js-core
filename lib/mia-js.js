...
function MiaJs() {
    ...
    const _applyServerOptions = (type, server, options) => {
        ...
        if (options && typeof options === 'object' && (options).constructor === Object && !_.isEmpty(options)) {
            Logger.info('[' + type + ' Server Settings]------------------------------------');
        ...
        try {
            if (httpsServerConfig) {
                ...
                if (options && options.key && options.cert && httpsServerConfig.port) {
                    ...
                    let server = await httpsServer.listen(httpsServerConfig.port);
                    _applyServerOptions('HTTPS', server, httpsServerConfig.options);
            ...
            if (httpServerConfig && httpServerConfig.port) {
                if (httpServerConfig.redir === true) {
                    ...
                    let server = await appHttp.listen(httpServerConfig.port);
                    _applyServerOptions('HTTP', server, httpServerConfig.options);
