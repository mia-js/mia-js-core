...
function MiaJs() {
    ...
    var _initRoutes = function (reInit = false) {
        ...
        _vhosts = RoutesHandler.initializeRoutes(_vhosts, reInit === false);
        return Q();
    };
    ...
    const _cronJobStarter = function () {
        const cronJobsToStart = _getNameOfCronjobsToStart();
        if (!cronJobsToStart && _shouldStartCrons() && Shared.isDbConnectionAvailable() === true) {
            return CronJobManagerJob.startListening().then(function () {
                Logger.tag('Cron').info('Cron Job Manager is started. Starting all possible crons');
                return Q();
            }, function (err) {
        ...
        else if (cronJobsToStart && _shouldStartCrons(cronJobsToStart)) {
                
            if (cronJobsToStart.length === 1) {
                const singleCronTaskName = cronJobsToStart[0];
                Logger.tag('Cron').info('Starting single cronjob "' + singleCronTaskName + '"');
                const cron = Shared.cronModules([singleCronTaskName])[0];
                if (!cron) {
                    process.exit(1);
                }
                try {
                ...
                return CronJobManagerJob.startListening(cronJobsToStart).then(() => {
                    Logger.tag('Cron').info('Starting specific cronjobs "' + cronJobsToStart.join(' , ') + '"');
     ...
     * @private
     */
    const _getNameOfCronjobsToStart = function () {
        if (process.argv[3] && (process.argv[3].indexOf('cron=') !== -1 || process.argv[3].indexOf('crons=') !== -1)) {
            const cronTasks = process.argv[3].split('=')[1].replace(/\s/g, "").split(',');
            return cronTasks.length > 0 ? cronTasks : undefined;
        }
        return undefined;
    };

    const _shouldStartCrons = (passedCronNames) => {
        let cronsTurnedOff = false;
        if(passedCronNames && passedCronNames.length > 0){
            if (['off', 'disabled', '0', 'false', 'disable'].includes(String(passedCronNames[0]).toLowerCase())){
                cronsTurnedOff = true;
            }
        }

        return cronsTurnedOff ? false : Shared.config("environment.cronJobs.enabled") === true //first check cronTurnedOff
    }
    ...
    self.init = function (initFunction) {
        ...
        return _getHost()
            ...
            .then(function () {
                const cronsToStart = _getNameOfCronjobsToStart();
                if (cronsToStart && _shouldStartCrons(cronsToStart)) {
