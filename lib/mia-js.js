...
function MiaJs() {
    ...
    const _cronJobStarter = function () {
        ...
        if (!cronJobsToStart && Shared.config("environment.cronJobs.enabled") === true && Shared.isDbConnectionAvailable() === true) {
            ...
            if (cronJobsToStart.length === 1) {
                ...
                return CronJobManagerJob.startListening(cronJobsToStart).then(() => {
                    Logger.tag('Cron').info('Starting specific cronjobs "' + singleCronTaskName.join(' , ') + '"');
                    Logger.tag('Cron').info('Cron Job Manager is started.');
                    return Q();
                }).catch(err => {
                    Logger.tag('Cron').error('Error starting Cron Job Manager ');
     ...
     * @private
     */
    var _getNameOfCronjobsToStart = function () {
        if (process.argv[3] && process.argv[3].indexOf('cron=') !== -1) {
            const cronTasks = process.argv[3].split('=').split(',');
