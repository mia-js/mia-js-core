...
function StaticDependencies(Logger, Shared, config) {
    ...
    const _writeOutHMRListeners = (stream, path, dependency, sharedFunctionName) => {
        ...
        switch (sharedFunctionName) {
            case 'config':
                stream.write(`if(module.hot)module.hot.accept('${dependency.absoluteFullPath}', () => {console.log('[HMR] Changed config file with identity "${dependency.identity}"'); ${variable} = require('${dependency.absoluteFullPath}'); Shared.setConfig([${path.map(p => "'" + p + "'").join(', ')}], ${variable})});\n`);
                break;
            case 'init':
